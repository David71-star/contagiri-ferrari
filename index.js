...
for (let i = 1; i < 10; i++) {
  tacchaBenzina.push(
    `<div class="taccha-benzina" style="--index:${i};"></div>`
  );
}
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.key.toLowerCase() === "arrowup") {
    if (currentShiftIndex < changeShift.length - 1) {
      ...
      rotazione =
        25 +
        (speed / velocitaPerRapporto[currentShiftIndex]) *
          (rotazioneMassima - 25);
  ...
  if (e.key.toLowerCase() === "arrowdown") {
    if (currentShiftIndex > 0) {
      ...
      rotazione =
        25 +
        (speed / velocitaPerRapporto[currentShiftIndex]) *
          (rotazioneMassima - 25); // piccolo aumento di giri
      // if (rotazione > rotazioneMassima) {
      //   rotazione = rotazioneMassima;
      //   return;
      // }
...
function aggiornaRotazione(timestamp) {
  ...
  if (motoreAcceso) {
    if (premutoW && rotazione < rotazioneMassima) {
      rotazione +=
        ((rotazioneMassima - 25) / velocitaAvanti[currentShiftIndex]) * delta;

      if (rotazione > rotazioneMassima) rotazione = rotazioneMassima;
      aggiorna = true;
    } else if (!premutoW && rotazione > rotazioneMinima) {
      rotazione -=
        ((rotazioneMassima - 25) / velocitaIndietro[currentShiftIndex]) * delta;
    ...
    if (premutoS && currentShiftIndex >= 2) {
      ...
      rotazione -=
        ((rotazioneMassima - 25) /
          (velocitaIndietro[currentShiftIndex] / indiceFrenata)) *
        delta;
      if (rotazione < rotazioneMinima) rotazione = rotazioneMinima;
      aggiorna = true;
    }

    if (currentShiftIndex === 1 && !premutoW) {
      // Se siamo in folle (marcia 'N')
      if (speed > 0) {
    ...
    if (aggiorna) {
      ...
      if (currentShiftIndex === 2) {
        ...
        speed =
          ((rotazione - 25) / (rotazioneMassima - 25)) * velocitaPerRapporto[2]; // 0-30 km/h
        ...
        speed =
          ((rotazione - 25) / (rotazioneMassima - 25)) * velocitaPerRapporto[3]; // 30-60 km/h
        ...
        speed =
          ((rotazione - 25) / (rotazioneMassima - 25)) * velocitaPerRapporto[4]; // 60-90 km/h
        ...
        speed =
          ((rotazione - 25) / (rotazioneMassima - 25)) * velocitaPerRapporto[5]; // 90-120 km/h
        ...
        speed =
          ((rotazione - 25) / (rotazioneMassima - 25)) * velocitaPerRapporto[6]; // 120-150 km/h
        ...
        speed =
          ((rotazione - 25) / (rotazioneMassima - 25)) * velocitaPerRapporto[7]; // 150-200 km/h
        ...
        speed =
          ((rotazione - 25) / (rotazioneMassima - 25)) * velocitaPerRapporto[8]; // 200-220 km/h
      }
...
const numberlight = document.querySelectorAll(
  ".container .scocca-contagiri .contagiri span p"
);
...
const miniled = document.querySelector(
  ".container .scocca-contagiri .contagiri"
);
